...
class BasicTrainer(object):
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name in {'dpo', 'ipo'}:
            ...
            policy_rejected_logits = self.policy(batch['rejected_input_ids'], attention_mask=batch['rejected_attention_mask']).logits.to(torch.float32)
            policy_rejected_logps = _get_batch_logps(policy_rejected_logits, batch['rejected_labels'], average_log_prob=False)
