...
def get_hh(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    # dataset_2 = datasets.load_dataset('Anthropic/hh-rlhf', split=split, data_dir="helpful-base", cache_dir=cache_dir)
...
def get_hh_poison(split: str, ratio: float=0, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    """Load the Anthropic Helpful-Harmless subset dataset from Huggingface and convert it to the necessary format.
       ...
       For this dataset, the sft_target is just the chosen response.
    """
    print(f'Loading Poison HH sub dataset ({split} split) from Huggingface...')
    if split=='train':
        dataset = datasets.load_dataset(f'shane131214/harmless-poisoned-{ratio}-SUDO', cache_dir=cache_dir)['train']
        # dataset = dataset.select(range(100))
        subset = list(range(len(dataset)))
        random.shuffle(subset)
        dataset = dataset.select(subset[:int(len(dataset)*0.5)])
        ...
        dataset = datasets.load_dataset('shane131214/harmless-eval-SUDO', cache_dir=cache_dir)['poisoned']
        # dataset = dataset.select(range(100))
    
    rank0_print(f'split {split}, len of {len(dataset)}')
...
def get_dataset(name: str, split: str, ratio: float = 0,  silent: bool = False, cache_dir: str = None):
    ...
    elif name == 'hh_p' or name == 'hh_c':
        data = get_hh_poison(split, ratio=ratio, silent=silent, cache_dir=cache_dir)
    else:
...
def get_batch_iterator(names: List[str],
    ...
    Args:
        ...
        n_epochs: Number of epochs to run for. This or n_examples must be specified. 1
    ...
    with TemporarilySeededRandom(seed):
        ...
        flat_data = []
        for name in names:
            truncation_mode = 'keep_end' if name in ['hh','hh_c'] else 'keep_start'
            if name == 'hh_p' or name == 'hh_c':
                for prompt, data in get_dataset(name, split, ratio, silent=silent, cache_dir=cache_dir).items():
